...
const FEATURES: { icon: string; title: string; sub: string; free?: boolean }[] = [
  { icon: '✅', title: 'Take roll in one tap', sub: 'Everyone starts present. Tap only the exceptions.', free: true },
  { icon: '📄', title: 'Attendance report PDF', sub: 'Per-person totals and the full register grid, ready to hand in.' },
  { icon: '👥', title: 'Unlimited groups and sessions', sub: 'Every class, team, room, or crew you run.' },
  { icon: '🧾', title: 'CSV export', sub: 'Every mark as a spreadsheet, yours to keep.' },
  { icon: '🔒', title: 'Face ID lock', sub: 'Your roster stays your business.' },
];
...
export default function PaywallModal({ privacyUrl }: { privacyUrl: string }) {
  ...
  const buy = async () => {
    ...
    if (res.ok && res.isPro) {
      setIsPro(true);
      hidePaywall();
      Alert.alert('You’re all set', 'Pro is on. Take roll, then export the report whenever you need it.');
      ...
      <View style={[styles.container, { backgroundColor: theme.bg }]}>
        <ScrollView contentContainerStyle={styles.scroll}>
          ...
          <Text style={[styles.title, { color: theme.text }]}>
            Take roll in seconds.{'\n'}Hand in the report.
          </Text>
          <Text style={[styles.subtitle, { color: theme.textSecondary }]}>
            Start with {weeklyTrial ?? '3 days free'}. Every group, every session, and the PDF
            attendance report your office or league asks for.
          </Text>
                  ...
                  >
                    <View style={{ flex: 1 }}>
                      <View style={styles.pkgTitleRow}>
                        ...
                        {isWeekly && (
                          <View style={[styles.popular, { backgroundColor: theme.highlight }]}>
        ...
        */}
        <View style={[styles.legal, { borderTopColor: theme.border, backgroundColor: theme.bg }]}>
          <Text style={[styles.legalText, { color: theme.textSecondary }]}>
            Attendance Tracker Pro. {disclosure}
          </Text>
...
const styles = StyleSheet.create({
  ...
  popularText: { fontSize: 9, fontWeight: fonts.weight.bold, color: '#141B2E', letterSpacing: 0.5 },
